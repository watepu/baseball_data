...
describe Pitcher do
  ...
  it "most_winners_in_teamメソッド使用時、最多勝投手が一人の場合" do
    ...
    expect(Pitcher.most_winners_in_team(2000, 2002)).to eq["siro"]
  end
  ...
  it "most_winners_in_teamメソッド使用時、最多勝投手が複数人の場合" do
    ...
    Pitcher.create(player_id: 'rokuro', year_id: 2003, wins: 21)
    expect(Pitcher.most_winners_in_team(2000, 2002)).to eq ["siro", "goro"]
  end

  it "most_winners_in_teamメソッド使用時、指定した期間のデータが存在しない場合" do
    Pitcher.create(player_id: 'taro', year_id: 1999, wins: 21)
    Pitcher.create(player_id: 'rokuro', year_id: 2003, wins: 21)
    expect(Pitcher.most_winners_in_team(2000, 2002)).to eq []
  end
